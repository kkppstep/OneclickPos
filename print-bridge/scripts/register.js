...
// Uses the exact same /hubs/register handshake as local-hub — a print
// bridge is just a hub row that only ever calls GET /orders/pending,
// POST /orders/:id/ack, and GET /stores/:id/settings, never the
// terminal-facing order-submission endpoints. Generate the code from
// the same "Hub setup" tab in admin-app you'd use for a full hub.
async function main() {
  const storeId = process.env.STORE_ID;
  const provisioningCode = process.argv[2];
  const deviceName = process.argv[3] || `${os.hostname()} (print bridge)`;
  ...
  try {
    ...
    console.log(`Set the printer's IP in admin-app's Hub setup tab, then run: npm start`);
  } catch (err) {
